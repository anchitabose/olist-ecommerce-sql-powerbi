...
A recursive CTE generated a complete month-by-month calendar, `LEFT JOIN`ed against actual orders to auto-detect zero-order months: **November 2016** (pre-launch phase) and **November 2018** (data extraction cutoff, not a real gap).
...
**Design:** white background, deep-green (`#1E7145`) and soft-mint (`#EAF3ED`) palette, consistent card styling across all panels — built as a single, dense page rather than a multi-page report, to keep the full story glanceable at once.
